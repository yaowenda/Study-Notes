...
```js
// 更新提交状态
export function updateSubmitStatus(docStatus, saleOutId) {
  return request({
    url: `/saleOut/saleOut/updateSubmitStatus?docStatus=${docStatus}&saleOutId=${saleOutId}`,
    method: 'put'
  })
}

// 更新审核状态
export function updateApprovalStatus(approvalStatus, saleOutId) {
  return request({
    url: `/saleOut/saleOut/updateApprovalStatus?approvalStatus=${approvalStatus}&saleOutId=${saleOutId}`,
    method: 'put'
  })
}
```
...
```xml
<update id="updateSubmitStatus" parameterType="map">
        update sale_order set doc_status = #{docStatus} where sale_order_id = #{saleOrderId}
    </update>

    <update id="updateApprovalStatus" parameterType="map">
        update sale_order set approval_status = #{approvalStatus} where sale_order_id = #{saleOrderId}
    </update>
```

```java
public int updateSubmitStatus(Map<String, Object> params);

    public int updateApprovalStatus(Map<String, Object> params);
```

```java
// 更新提交状态
    @Override
    public int updateSubmitStatus(String docStatus, Long saleOrderId){
        
        Map<String, Object> params = new HashMap<>();
        params.put("docStatus", docStatus);
        params.put("saleOrderId", saleOrderId);
        return saleOrderMapper.updateSubmitStatus(params);
    }

    // 更新审核状态
    @Override
    public int updateApprovalStatus(String approvalStatus, Long saleOrderId){
        Map<String, Object> params = new HashMap<>();
        params.put("approvalStatus", approvalStatus);
        params.put("saleOrderId", saleOrderId);
        return saleOrderMapper.updateApprovalStatus(params);
    }
```

```java
@PutMapping("/updateSubmitStatus")
    public int updateSubmitStatus(@RequestParam String docStatus, @RequestParam Long saleRefundId){
        return saleRefundService.updateSubmitStatus(docStatus, saleRefundId);
    }

    @PutMapping("/updateApprovalStatus")
    public int updateApprovalStatus(@RequestParam String approvalStatus, @RequestParam Long saleRefundId){
        return saleRefundService.updateApprovalStatus(approvalStatus, saleRefundId);
    }
```
...
```js
// 保存审核记录
export function saveApprovalRecord(payload) {
  return request({
    url: '/refund/refund/saveApprovalRecord',
    method: 'put',
    data: payload
  })
}
...
<el-button size="mini" type="text" icon="el-icon-s-order"
            @click="showApprovalRecord(scope.row)">审核记录</el-button>
```

```js
// 点击“查看审核记录”按钮
    showApprovalRecord(row) {
      this.approvalRecordDialogVisible = true;
      // 请求该单据的审核记录
      getApprovalRecordByReturnId(row.returnId).then(res => {
        this.approvalRecordList = res || [];
      });
    },
```
...
```js
export function getApprovalRecordByReturnId(returnId) {
  return request({
    url: '/returnApproval/returnApproval/getApprovalRecordByReturnId',
    method: 'get',
    params: { returnId }
  })
}
...
```xml
<select id="getApprovalRecordBySaleOrderId" parameterType="Long" resultMap="SaleOrderApprovalRecordResult">
        select * from sale_order_approval_record where sale_order_id = #{saleOrderId}
    </select>
```

```java
public List<SaleOrderApprovalRecord> getApprovalRecordBySaleOrderId(Long saleOrderId);
```

```
@Override
    public List<SaleOrderApprovalRecord> getApprovalRecordBySaleOrderId(Long saleOrderId){
        return saleOrderApprovalRecordMapper.getApprovalRecordBySaleOrderId(saleOrderId);
    }
```
